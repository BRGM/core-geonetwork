...
DROP FUNCTION IF EXISTS mgs_migrate(character varying,integer,character varying,character varying,character varying,character varying);
...
DROP FUNCTION IF EXISTS mgs_migrate(int, character varying,integer,character varying,character varying,character varying,character varying);

CREATE OR REPLACE FUNCTION mgs_migrate_single(
    nodeToMigrate int,
    dbhost varchar DEFAULT 'localhost',
    dbport int DEFAULT 5432,
    dbusername varchar DEFAULT 'www-data',
...
BEGIN
    ...
    FOR rec IN SELECT id::int, name, path, username, password
               FROM nodes WHERE id::int = nodeToMigrate::int
               ORDER BY id
        LOOP
            userIdSeq := 0;
            recordIdSeq := 0;
            ...
            EXECUTE format(
                    'INSERT INTO mgs_tmp_siteids (node, uuid) SELECT * FROM dblink(''%s'', '''
                        ...
                        ') AS t1(node int, uuid varchar)',
                    dblink, rec.id);
            INSERT INTO groups (id, name, logo) VALUES (rec.id, rec.id, 'logo_' || rec.id || '.png');
            RAISE NOTICE '#% Set group label ...', rec.id;
            EXECUTE format(
                    'INSERT INTO groupsdes (iddes, label, langid) VALUES (%s, ''%s'', ''fre'')',
                    rec.id, REPLACE(rec.name, '''', ''''''));
            ...
            -- TODO: Logo
            INSERT INTO sources (uuid, creationdate, filter, logo, name, type, uiconfig)
            VALUES (rec.id, '2019-10-01T11:11:30', concat('+_groupPublished:', rec.id), concat('logo_', rec.id, '.png'), rec.name, 'subportal', null);
            ...
            UPDATE metadata SET schemaId = 'iso19139' WHERE schemaid = 'iso19139.fra' AND groupowner = rec.id;
            -- Update GML namespace for moving from ISO19139:2005 to ISO19139:2007
            UPDATE Metadata SET data = replace(data, '"http://www.opengis.net/gml"', '"http://www.opengis.net/gml/3.2"') WHERE data LIKE '%"http://www.opengis.net/gml"%' AND schemaid = 'iso19139' AND groupowner = rec.id;

            UPDATE metadata SET data = replace(data, '<gmd:version gco:nilReason="missing">', '<gmd:version gco:nilReason="unknown">') WHERE  data LIKE '%<gmd:version gco:nilReason="missing">%' AND groupowner = rec.id;

            UPDATE Metadata SET data = replace(data, 'http://standards.iso.org/ittf/PubliclyAvailableStandards/ISO_19139_Schemas/resources/codelist/ML_gmxCodelists.xml', 'http://standards.iso.org/iso/19139/resources/gmxCodelists.xml') WHERE data LIKE '%http://standards.iso.org/ittf/PubliclyAvailableStandards/ISO_19139_Schemas/resources/codelist/ML_gmxCodelists.xml%' AND schemaid = 'iso19139' AND groupowner = rec.id;

            -- Unset 2005 schemaLocation
            UPDATE Metadata SET data = regexp_replace(data, ' xsi:schemaLocation="[A-Za-z0-9 \/:\.]*"', '') WHERE data LIKE '%xsi:schemaLocation=%' AND groupowner = rec.id;

            UPDATE metadata SET data = replace(data, 'Commission RÈGLEMENT (UE) N o 1089/2010 DE LA COMMISSION du 23 novembre 2010 portant modalités d''application de la directive 2007/2/CE du Parlement européen et du Conseil en ce qui concerne l''interopérabilité des séries et des services de données géographiques', 'RÈGLEMENT (UE) N o 1089/2010 DE LA COMMISSION du 23 novembre 2010 portant modalités d''application de la directive 2007/2/CE du Parlement européen et du Conseil en ce qui concerne l''interopérabilité des séries et des services de données géographiques') WHERE  data LIKE '%Commission RÈGLEMENT (UE) N o 1089/2010 DE LA COMMISSION du 23 novembre 2010 portant modalités d''application de la directive 2007/2/CE du Parlement européen et du Conseil en ce qui concerne l''interopérabilité des séries et des services de données géographiques%' AND groupowner = rec.id;

            INSERT INTO sourcesdes (iddes, langid, label) (
                SELECT uuid, 'fre', g.label FROM sources s, groupsdes g, groups b
                WHERE s.type = 'subportal' AND s.uuid = rec.id::text AND b.name = s.uuid AND b.id = g.iddes AND g.langid = 'fre' AND concat(uuid, 'fre') NOT IN (SELECT concat(iddes, 'fre') FROM sourcesdes)
                UNION ALL
                SELECT uuid, 'eng', g.label FROM sources s, groupsdes g, groups b
                WHERE s.type = 'subportal' AND s.uuid = rec.id::text AND b.name = s.uuid AND b.id = g.iddes AND g.langid = 'eng' AND concat(uuid, 'eng') NOT IN (SELECT concat(iddes, 'eng') FROM sourcesdes));

            UPDATE sources s SET groupowner = (SELECT g.id FROM groups g WHERE g.name = s.uuid) WHERE s.type = 'subportal' AND s.uuid = rec.id::text;
        END LOOP;
