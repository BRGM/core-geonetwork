...
public class BrgmStatusActions implements StatusActions {
    ...
    protected List<User> getUserToNotify(MetadataStatus status) {
        ...
        if (notificationLevel != null) {
            if (notificationLevel == StatusValueNotificationLevel.statusUserOwner) {
                User owner = userRepository.findOne(status.getOwner());
                users.add(owner);
            } else if (notificationLevel == StatusValueNotificationLevel.recordProfileReviewerOfAllEditGroups) {

                // BRGM start - get all operations allowed for a record
                OperationAllowedRepository operationAllowedRepository = context.getBean(OperationAllowedRepository.class);
                final List<OperationAllowed> operations = operationAllowedRepository.findAllById_MetadataId((Integer) listOfId.toArray()[0]);
                List<Integer> groupsWhichCanEdit = new ArrayList<>();
                operations.forEach(o -> {
                    if (o.getId().getOperationId() == ReservedOperation.editing.getId()) {
                        groupsWhichCanEdit.add(o.getId().getGroupId());
                    }
                });
                ...
                if (groupsWhichCanEdit.size() > 0) {
                    ...
                    Specifications<UserGroup> userGroupSpecifications =
                        where(hasProfile(Profile.Reviewer)).and(hasGroupIds(groupsWhichCanEdit));
